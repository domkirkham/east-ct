...
class Xtreme(object):
    def __init__(self, file):
        ...
        if self.okay:
            ...
            res = h[19] / h[7]
            skip_samples = math.floor(30.0 / res)
            ...
            self.skip_angles = 2 * math.floor(9 / res)
            self.skip_samples = skip_samples

            self.fan_scans = math.floor(h[20] / res)
            self.fan_angles = math.floor(168 / res - self.skip_angles)
    ...
    def fan_to_parallel(self, X):
        ...
        atheta = self.fan_theta / 6  # 1/6 of the fan angle
        ...
        #    yo1[index] = index[0]
        yo = np.arcsin((xo1 - (samples / 2)) / self.radius)
